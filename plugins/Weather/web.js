...
module.exports.get = function(guildID) {
    return new Promise(async (res, rej) => {
        const data = await db.getGuildPluginAliasesAndPerms(guildID, "Weather", {"weather": ["weather"]}, {"weather":["@everyone"]});
...
module.exports.put = function(guildID, data) {
    return new Promise(async (res, rej) => {
        Discord.getGuildRoles(guildID).then(roles => {
            ...
            db.setGuildPluginAliases(guildID, "Weather", aliases);
            db.setGuildPluginPerms(guildID, "Weather", perms);
            res();
        });
    });
}
