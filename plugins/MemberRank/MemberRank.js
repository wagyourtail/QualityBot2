    ...
    return new Promise((resolve, reject) => {
        client.database.getGuildPluginData(guild.id, "MemberRank", {static:{}, dynamic:{}}).then(async (ranks) => {
            ...
            for (const [rank, role] of Object.entries(ranks.static)) {
                if (userRank.rank <= rank && guild.roles.resolve(role) && !member.roles.cache.has(role)) member.roles.add(role);
                if (userRank.rank > rank && guild.roles.resolve(role) && member.roles.cache.has(role)) member.roles.remove(role);
            }
            ...
            for (const [rank, role] of Object.entries(ranks.dynamic)) {
                if (userRank.rank <= (userCount * rank / 100) && guild.roles.resolve(role) && !member.roles.cache.has(role)) member.roles.add(role);
                if (userRank.rank > (userCount * rank / 100) && guild.roles.resolve(role) && member.roles.cache.has(role)) member.roles.remove(role);
            }
