...
module.exports.get = function(guildID) {
    return new Promise(async (res, rej) => {
        const data = await db.getGuildPluginAliasesAndPerms(guildID, "PluginName", {"commandname": ["commandname", "aliases"]}, {"commandname":[]}); //first object is default aliases, second is default permissions (either @everyone or nothing).
...
module.exports.put = function(guildID, data) {
    return new Promise(async (res, rej) => {
        Discord.getGuildRoles(guildID).then(roles => {
            ...
            db.setGuildPluginAliases(guildID, "PluginName", aliases);
            db.setGuildPluginPerms(guildID, "PluginName", perms);
            res();
        });
    });
}