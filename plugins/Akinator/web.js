...
module.exports.get = function(guildID) {
    return new Promise(async (res, rej) => {
        const data = await db.getGuildPluginAliasesAndPerms(guildID, "Akinator", {"akinator": ["akinator"]}, {"akinator":["@everyone"]});
...
module.exports.put = function(guildID, data) {
    return new Promise(async (res, rej) => {
        Discord.getGuildRoles(guildID).then(roles => {
            ...
            db.setGuildPluginAliases(guildID, "Akinator", aliases);
            db.setGuildPluginPerms(guildID, "Akinator", perms);
            res();
        });
    });
}
